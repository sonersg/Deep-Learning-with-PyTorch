...
# Set custom NLTK data dir inside your project
...
# Add to NLTK's search path (highest priority)
if nltk_data_dir not in nltk.data.path:
    nltk.data.path.insert(0, nltk_data_dir)

# Download only if missing
required_resources = ['punkt', 'punkt_tab', 'wordnet']
...
class ChatbotAssistant:
    ...
    def process_message(self, input_message, confidence_threshold=0.7):
        ...
        if max_probability < confidence_threshold:
            return f"Sorry, I'm not trained for that kind of naughty things. {max_probability}"
        ...
        if self.intents_responses[predicted_intent]:
            return f"{random.choice(self.intents_responses[predicted_intent])} {max_probability:.1f}"
        else:
            return None
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(f"Model: {assistant.process_message(message)}")
